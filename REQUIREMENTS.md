...
| REQ-008 | planned     | Analytics     | System ma definiować okazję relatywnie do porównywalnych ofert, a nie przez stały próg ceny.      | Porównanie ma uwzględniać co najmniej rocznik, przebieg, silnik, paliwo i skrzynię.                              |
| REQ-009 | planned     | Analytics     | Warstwa analityczna v1 ma wyliczać wstępny score tylko na podstawie danych dostępnych z listingu. | Bez wymagania wejścia w szczegóły każdej oferty.                                                                 |
| REQ-010 | in-progress | Analytics     | System ma grupować oferty do porównań w segmenty podobnych pojazdów.                              | Segment powinien uwzględniać model z danej kwerendy oraz tolerancje dla rocznika, przebiegu i parametrów napędu. |
| REQ-011 | in-progress | Analytics     | Wstępny score ma składać się z wielu sygnałów, a nie tylko z samej ceny.                          | Co najmniej: relacja do rynku, świeżość oferty, zmiany ceny i kompletność danych.                                |
| REQ-012 | in-progress | Analytics     | System ma zwracać wynik analityczny w postaci score oraz listy powodów, które wpłynęły na ocenę.  | Wynik ma być zrozumiały przed wdrożeniem LLM.                                                                    |
| REQ-013 | in-progress | Enrichment    | System ma kierować do enrichmentu tylko oferty spełniające reguły priorytetyzacji.                | Co najmniej: nowe oferty, istotny spadek ceny, wysoki score lub braki ważnych danych.                            |
| REQ-014 | in-progress | Enrichment    | System nie ma pobierać szczegółów każdej oferty przy każdym przebiegu.                            | Enrichment ma być selektywny i zależny od opłacalności pobrania dodatkowych danych.                              |
| REQ-015 | in-progress | LLM           | Do warstwy LLM mają trafiać tylko oferty wyselekcjonowane przez scoring i enrichment.             | LLM nie powinien analizować pełnego zbioru ofert.                                                                |
...
| REQ-021 | planned     | Analytics     | `deal_score` v1 ma być liczbą znormalizowaną do zakresu `0-100`.                                  | Wyższy wynik oznacza większe prawdopodobieństwo okazji.                                                          |
...
| REQ-025 | planned     | Analytics     | Wynik analityczny ma zawierać osobno `deal_score` i `confidence_score`.                           | Te wartości nie mogą być zlane w jeden nieczytelny wskaźnik.                                                     |
| REQ-026 | planned     | Analytics     | Wynik analityczny ma klasyfikować oferty do poziomów decyzji.                                     | Co najmniej: `ignore`, `watch`, `candidate`, `high-priority`.                                                    |
