...
| REQ-016 | done        | Notifications | Powiadomienia mają być wysyłane tylko dla nowych lub istotnie zmienionych okazji.                                              | Implemented: eventy `new-listing`/`reactivated`/`bucket-upgrade`/`price-drop` z dodatkowymi filtrami antyspamowymi (progi confidence, ograniczenia reactivated i bucket-upgrade).       |
