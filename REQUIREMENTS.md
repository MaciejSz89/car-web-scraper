...
| REQ-029 | in-progress | Enrichment    | System ma ograniczać częstotliwość ponownego enrichmentu tej samej oferty.                                          | Implemented basic skip dla `fetched` i retry tylko dla `failed`; brak jeszcze cooldownu po zmianach ceny/stanu.                       |
| REQ-030 | in-progress | Enrichment    | Wynik enrichmentu ma zapisywać moment pobrania i stan oferty, dla którego pobrano szczegóły.                        | Implemented: `details_status`, `details_priority`, `details_fetched_at` w CSV; brak jeszcze pełnego `details_based_on_*` kontraktu.   |
