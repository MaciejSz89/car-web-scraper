...
- LLM przestał wpływać na decyzje o powiadomieniach — usunięto blokowanie przez `llm_verdict=reject`/`llm_risk_level=high` (REQ-052 removed) oraz event `llm-approved` (REQ-053 removed). LLM pełni wyłącznie rolę komentatora: ocena jest dołączana do treści powiadomienia, ale nie blokuje ani nie wyzwala eventów.
- Poprawiono jakość komentarzy LLM: `summary` zmienione z jednego zdania na 3-5 zdań (ocena cenowa, sygnały, rekomendacja), liczba powodów zwiększona do 8, `DEFAULT_MAX_TOKENS` podniesiony do 1600, system prompt uzupełniony o instrukcję konkretności (REQ-058 updated).
- Naprawiono błąd scrapera: `wait_until_article_count_stabilizes()` w `scraper.py` wychodziła po 2 rundach z zerową liczbą artykułów, zanim React zamontował listingi. Dodano `page.wait_for_selector("article[data-id]", timeout=20000)` przed pętlą stabilizacji.
...
- 2026-04-15: Naprawiono fałszywe powiadomienia dla ofert, gdzie kwota z ogłoszenia była ratą lub elementem cesji, a nie realną ceną auta. Rozszerzono i scalono domyślne flagi uszkodzeń z konfiguracją użytkownika w filtrze powiadomień. Dodano testy regresyjne.
- 2026-04-17: Naprawiono paginację scrapera — selektor `div.eemmnsu4` (auto-generowana klasa CSS Otomoto) przestał działać po aktualizacji frontendu serwisu. Zastąpiony stabilnymi selektorami atrybutowymi `button[title='Go to next Page']` / `button[aria-label='Go to next Page']`.
- 2026-04-17: Naprawiono `AttributeError: 'NoneType' has no attribute 'strip'` w `enrichment_worker.py` przy odczycie `details_status` — wartość None jest teraz bezpiecznie traktowana jako pusty string.
...
- 2026-04-19: Naprawiono lukę w wykrywaniu uszkodzonych ofert — strukturalna flaga „Uszkodzony" z sekcji „Stan i historia" (checkbox `parametersDict["damaged"]` z `__NEXT_DATA__`) nie była wcześniej uwzględniana w scoringu ani filtrze powiadomień. Dodano `_score_parameters()` w `enrichment_analysis.py` z karą -25 pkt i flagą `damage_structural`; analogicznie premia +8 pkt i flaga `accident_free_structural` dla `no_accident=True`. Nowe pole `details_damaged_flag` w CSV (OR tekstu i struktury); `notifications.py` sprawdza to pole obok `is_damaged`.
- 2026-04-19: Dodano tryb `damaged_handling=llm` w `notification_filters` (REQ-055): drobne uszkodzenia delegowane do oceny LLM — powiadomienie tylko przy `llm_verdict=approve` + ryzyko non-high; ciężkie flagi (szkoda całkowita, wystrzelone poduszki itd.) zawsze twardy blok. Prompt LLM rozszerzony o sekcję `## Stan uszkodzenia`.
- 2026-04-19: Dodano flagę `--loop` w `main.py` (REQ-056): nieprzerwana pętla scrapowania z konfigurowalnymi przerwami (`--loop-interval SEC`, domyślnie 1800 s). Ctrl+C przerywa czysto w dowolnym momencie.
- 2026-04-19: Naprawiono detekcję importu w `enrichment_analysis._score_consistency()` — poprzedni kod nie wykrywał wzorca checkboxa otomoto (`value == key`). Dodano regułę `overseas_import` z karą -8 pkt dla rynków zamorskich (REQ-057). Prompt LLM uzupełniony o sekcję `## Import i kraj pochodzenia`. Naprawiono też problem przestarzałych rekordów CSV: `build_prompt()` teraz czyta `parameters` bezpośrednio z sidecar JSON jako fallback — uszkodzenie i import są zawsze widoczne dla LLM niezależnie od wieku enrichmentu.
...
| 2026-04-18 | Dodano obsługę HTTP 410 Gone w `enrichment_worker.py`: status `gone` oddzielony od `failed`, automatyczny flush wpisów `gone` z kolejki, oznaczenie `is_active=0` w storage CSV i pominięcie `gone` w kolejnych przebiegach bez potrzeby `--retry-failed`.                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-04-18 | Zaimplementowano warstwę LLM (`llm_worker.py`): selekcja kandydatów po enrichmentcie, prompt po polsku z danymi oferty i sygnałami analityki, wywołanie OpenAI API z `response_format=json_object`, zapis wyniku (`llm_verdict`, `llm_risk_level`, `llm_confidence`, `llm_summary`, `llm_reasons`, `llm_reviewed_at`) do storage CSV. Zabezpieczenia: hard cap 20, `max_candidates_per_run` (domyślnie 5), cooldown 30 dni, `min_final_score`, wymóg sidecar JSON, przerwanie przy `RateLimitError`. Model konfigurowalny przez `preferences.json` (klucz `llm`). Integracja z `main.py` przez `--run-llm`, `--llm-limit`, `--llm-model`.                             |
| 2026-04-19 | Dodano `retry_failed_notifications()` w `notifications.py`: ponawia dostarczenie powiadomień oznaczonych `notification_status=failed` w `notification_history.csv` bez modyfikowania `notification_state.csv`. Dostępne przez `--retry-failed-notifications` w `main.py`.                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-04-19 | Dodano mechanizm per-query override filtrów powiadomień w `preferences.json` (klucz `queries.<nazwa_kwerendy>.notification_filters`). Umożliwia poluzowanie lub zaostrzenie progów (np. `min_confidence_score`) dla modeli z małą grupą porównawczą (np. Honda Jazz).                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-04-19 | Poprawka `_calculate_confidence_score` w `analytics.py`: kara za `fallback_level` jest teraz skalowana przez rozmiar grupy porównawczej (`size_factor = max(0.2, 1 - (group_size - MIN) / 30)`). Modele z małym rynkiem (Jazz, Golf Sportsvan) — bez zmian. Modele z dużą grupą mimo fallbacku (Sportage gs=165: conf 64→93, Qashqai gs=88: conf 76→95) przestają blokować realne okazje.                                                                                                                                                                                                                                                                             |
| 2026-04-19 | Zintegrowano wynik LLM z warstwą powiadomień: (1) `_is_notification_eligible()` blokuje oferty z `llm_verdict=reject` lub `llm_risk_level=high` (domyślnie włączone, konfigurowalne przez `block_llm_rejected`/`block_llm_high_risk` w `notification_filters`); (2) dodano event type `llm-approved` — fires gdy `llm_verdict=approve` + `llm_risk_level in {low,medium}` i nie wysłano wcześniej; deduplikacja przez nowe pole `llm_notified_verdict` w `notification_state.csv`; (3) `llm_summary` dołączany do treści wiadomości i pola `notification_reason_summary`; (4) dodano 4 testy regresyjne w `test_notifications.py`.                                    |
| 2026-04-22 | Naprawiono błąd normalizacji `parametersDict` z Otomoto (REQ-060): `_normalize_parameters()` w `enrichment_worker.py` brał `.label` zamiast `.values[0].value`, co powodowało `"damaged": "damaged"` i oznaczało wszystkie oferty jako uszkodzone. Naprawiono: (1) `_normalize_parameters()` wyciąga wartości z listy `values`; (2) guard `key==value → None` w `_parameter_flag()`; (3) analogiczny guard w `_score_parameters()` w `enrichment_analysis.py`; (4) guard w `build_prompt()` w `llm_worker.py`; (5) dodano `--reprocess-details` w `main.py` wywołujący `reprocess_details_flags()` — jednorazowa migracja 4787 wierszy CSV bez ponownego scrapowania. |
| 2026-04-22 | Zaplanowano wsparcie dla wielu źródeł danych (REQ-061–064): pole `source` w `queries.json`, wydzielenie scrapera i parsera do `scrapers/` i `parsers/`, nowe moduły `scrapers/mobile_de.py` i `parsers/mobile_de.py`. Warstwy storage/analytics/notifications nie wymagają zmian. Enrichment mobile.de odkładany do osobnej iteracji (REQ-064).                                                                                                                                                                                                                                                                                                                       |
| 2026-04-22 | Zaplanowano strukturyzację kwerend Otomoto (REQ-065): klucz `otomoto_params` w `queries.json` z polami `make`, `model`, `year_from`, `fuel_type`, `mileage_to` itp. — config loader buduje `start_url` w czasie ładowania. Pole `start_url` pozostaje jako fallback dla wstecznej kompatybilności.                                                                                                                                                                                                                                                                                                                                                                    |
