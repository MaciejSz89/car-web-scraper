...
| REQ-052 | done   | Notifications | Wynik LLM ma blokować powiadomienia dla ofert odrzuconych lub wysokiego ryzyka.                                                      | Implemented: `_is_notification_eligible()` sprawdza `llm_verdict=reject` (flaga `block_llm_rejected`, domyślnie True) i `llm_risk_level=high` (flaga `block_llm_high_risk`, domyślnie True); obydwie flagi konfigurowalne przez `notification_filters` w `preferences.json`. Oferty bez wyniku LLM (brak `llm_verdict`) nie są blokowane.                                                                                                                                                                                                                                                                                                                                                      |
| REQ-053 | done   | Notifications | Wynik LLM `approve` niskiego ryzyka ma generować event powiadomienia i być deduplikowany.                                            | Implemented: `determine_notification_event()` zwraca `llm-approved` gdy `llm_verdict=approve` i `llm_risk_level in {low,medium}` i wcześniej nie wysłano powiadomienia `llm-approved`; deduplikacja przez pole `llm_notified_verdict` w `notification_state.csv`; `llm_summary` dołączany do treści powiadomienia i historii.                                                                                                                                                                                                                                                                                                                                                                  |
...
| REQ-058 | done   | Notifications | Każde powiadomienie ma być opatrzone komentarzem LLM, jeśli oferta nie ma jeszcze oceny LLM.                                         | Implemented: `llm_worker.review_single()` + `llm_worker.OnDemandSession` enkapsulują całą logikę on-demand w warstwie LLM. `notifications.py` wywołuje wyłącznie te publiczne API. Limit wywołań per run konfigurowany przez `llm.max_notification_llm_calls` w `preferences.json` (domyślnie 5). Po ocenie on-demand następuje ponowna weryfikacja eligibility — `reject`/`high risk` blokuje powiadomienie; `approve + low/medium` ustawia `llm_notified_verdict=approve` zapobiegając duplikacji eventu `llm-approved` w następnym przebiegu.                                                                                                                                               |
